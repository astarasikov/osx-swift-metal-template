...
static DepthTestResult dist_round_box(float3 position, float3 box_pos) {
    float3 dd = abs(position) - box_pos;
    
    return {
        length(max(dd, 0.0)) - 0.2,
        float3(0.0, 0.0, 1.0)
    };
}
...
static DepthTestResult dist_to_scene(float3 position) {
    DepthTestResult s1 = dist_sphere(position, sphere1_pos, 0.6);
    DepthTestResult s2 = dist_sphere(position, sphere2_pos, 0.4);
    DepthTestResult fl = dist_floor(position);
    
    s2.color = float3(1.0, 1.0, 0.0);
    DepthTestResult box1 = dist_round_box(position, box1_pos);
...
static inline float4 trace(InterShaderData inData)
{
    ...
    //return float4(0.0, sin(inData.seed), 0.0, 1.0);
    ...
    float3 curPosOffseted = float3(dTest.depth) - float3(
        dist_to_scene(curPos + offt.xyy).depth,
        dist_to_scene(curPos + offt.yxy).depth,
        dist_to_scene(curPos + offt.yyx).depth
    );
